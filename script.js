const STORAGE_KEY = "stalkernet_pda_v395_archive_tidy";
...
function renderLore() {
  ...
  if (!entries.length) {
    list.innerHTML = `<article class="archive-entry module-panel"><div class="module-label">NO SIGNAL</div><p class="message-text">No archive records match your search. Try a broader word, category, location, or threat.</p></article>`;
    return;
  }
...
// v3.9.5 archive tidy pass
function tidyArchiveAfterRenderV395() {
  const archiveTab = document.getElementById("archiveTab");
  if (!archiveTab) return;

  archiveTab.querySelectorAll("button").forEach(btn => {
    const text = (btn.textContent || "").trim().toLowerCase();
    if (text === "open") btn.setAttribute("aria-label", "Open archive record");
    if (text === "close") btn.setAttribute("aria-label", "Close archive record");
  });

  archiveTab.querySelectorAll(".archive-source, .source-note, blockquote").forEach(note => {
    if (!note.dataset.tidiedV395) {
      note.dataset.tidiedV395 = "true";
      note.classList.add("archive-reference-note");
    }
  });

  archiveTab.querySelectorAll(".module-label").forEach(label => {
    if ((label.textContent || "").trim().toLowerCase() === "no signal") {
...
window.addEventListener("load", () => {
  setTimeout(tidyArchiveAfterRenderV395, 250);
  setTimeout(tidyArchiveAfterRenderV395, 900);
});

document.addEventListener("click", event => {
  const target = event.target;
  if (!target || !target.closest) return;
  if (target.closest("#archiveTab") || target.closest('[data-tab="archiveTab"]')) {
    setTimeout(tidyArchiveAfterRenderV395, 80);
  }
});

document.addEventListener("input", event => {
  const target = event.target;
  if (target && target.id === "archiveSearch") {
    setTimeout(tidyArchiveAfterRenderV395, 80);
  }
});
